...
# This script is the canonical standalone dispatcher; external orchestrators must
# preserve the same per-issue contract and claim semantics without reusing this
# script as the source of truth for cross-worker lease state.
#
...
CLI_RESULT_VALIDATOR="$SCRIPT_DIR/validate-cli-issue-result.js"
...
AGENT_CMD="scripts/configured-issue-agent.sh"
WORKDIR="$REPO_ROOT"
...
PROCESS_CALIBRATION_UPDATE="true"
CALIBRATION_UPDATE_CMD="scripts/update-calibration-store.sh"
...
Options:
  ...
  --workflow-mode <mode> independent|human-in-the-loop (default: independent)
                       Review behavior only; dispatch ownership is defined by
                       contracts/ralph/core/dispatch-protocol.md
  --dispatch-mode <mode> standalone|orchestrated (default: standalone)
                       standalone: ralph-run owns selection, loop state, retry
                       orchestrated: single-issue executor; orchestrator owns queue/lease
  --dispatch-id <id>   Dispatch identifier (required in orchestrated mode)
  --dispatch-run-id <id> Run identifier from orchestrator (required in orchestrated mode)
  --dispatch-log <path> Dispatch event log path (default: stdout in orchestrated mode)
  ...
  --agent-cmd <command> CLI command for one issue run (default: scripts/configured-issue-agent.sh)
  ...
  --calibration <path>     Calibration store path (default: .cursor/ralph/calibration.json)
  --process-calibration-update <bool> Update calibration store after retrospective (default: true)
  --calibration-update-cmd <command> Calibration updater command (default: scripts/update-calibration-store.sh)
...
is_workflow_mode() {
  [ "$1" = "independent" ] || [ "$1" = "human-in-the-loop" ]
}

is_dispatch_mode() {
  [ "$1" = "standalone" ] || [ "$1" = "orchestrated" ]
}

validate_dispatch_mode() {
  is_dispatch_mode "$DISPATCH_MODE" || fail "--dispatch-mode must be standalone|orchestrated"

  if [ "$DISPATCH_MODE" = "orchestrated" ]; then
    [ -n "$DISPATCH_ID" ] || fail "--dispatch-id is required in orchestrated mode"
    [ -n "$DISPATCH_RUN_ID" ] || fail "--dispatch-run-id is required in orchestrated mode"
  fi
}
...
emit_dispatch_event() {
  ...
  if [ -n "$DISPATCH_LOG_ABS" ]; then
    printf '%s\n' "$payload" >> "$DISPATCH_LOG_ABS"
  else
    printf '%s\n' "$payload"
  fi
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --process-calibration-update)
      shift
      PROCESS_CALIBRATION_UPDATE="${1:-}"
      ;;
    --calibration-update-cmd)
      shift
      CALIBRATION_UPDATE_CMD="${1:-}"
      ;;
...
is_workflow_mode "$WORKFLOW_MODE" || fail "--workflow-mode must be independent|human-in-the-loop"
...
is_bool "$PROCESS_CALIBRATION_UPDATE" || fail "--process-calibration-update must be true|false"
...
if [ "$WORKFLOW_MODE" = "human-in-the-loop" ]; then
  PROCESS_REVIEW_FEEDBACK_SWEEP="false"
fi
...
CALIBRATION_ABS="$(abs_path "$CALIBRATION_PATH")"
MODEL_ROUTING_POLICY_ABS="$(abs_path "$MODEL_ROUTING_POLICY_PATH")"

DISPATCH_LOG_ABS=""
if [ -n "$DISPATCH_LOG_PATH" ]; then
  DISPATCH_LOG_ABS="$(abs_path "$DISPATCH_LOG_PATH")"
fi
...
mkdir -p "$(dirname "$CALIBRATION_ABS")"
if [ "$RUN_LOG_ENABLED" = "true" ]; then
  mkdir -p "$(dirname "$RUN_LOG_ABS")"
fi
if [ -n "$DISPATCH_LOG_ABS" ]; then
  mkdir -p "$(dirname "$DISPATCH_LOG_ABS")"
fi
touch "$PROGRESS_ABS"
...
read -r -a CALIBRATION_UPDATE_CMD_ARR <<< "$CALIBRATION_UPDATE_CMD"
if [ "${#CALIBRATION_UPDATE_CMD_ARR[@]}" -eq 0 ]; then
  fail "invalid --calibration-update-cmd value"
fi
if [ "$PROCESS_CALIBRATION_UPDATE" = "true" ] && ! command -v "${CALIBRATION_UPDATE_CMD_ARR[0]}" >/dev/null 2>&1; then
  fail "calibration update command not found: ${CALIBRATION_UPDATE_CMD_ARR[0]}"
fi
...
CALIBRATION_SUMMARY_JSON='{"updated":false,"path":null,"samples_total":0,"usable_samples":0,"tokens_per_point":null,"worker_exit_code":null}'
...
next_issue() {
  jq -c --argjson blocked "$blocked_issues_json" '
    ...
    | [
        ...
        | ($description | gsub("\\r"; "")) as $description_normalized
        | ($description_normalized | gsub("^\\s+|\\s+$"; "")) as $description_trimmed
        | (($description_trimmed | length) > 0) as $has_description
        | ($description_trimmed | test("(?im)^##\\s*(goal|context)\\b")) as $has_goal_or_context
        | ($description_trimmed | test("(?im)^##\\s*(scope|implementation notes|implementation plan|approach|non-goals|constraints)\\b")) as $has_scope_signal
        | ($description_trimmed | test("(?im)^##\\s*acceptance criteria\\b")) as $has_acceptance_heading
        | ($description_trimmed | test("(?m)^\\s*[-*]\\s*\\[[ xX]\\]\\s+")) as $has_acceptance_checklist
        | ($description_trimmed | test("(?im)^##\\s*validation\\b")) as $has_validation_heading
        | ($description_trimmed | test("(?im)^\\s*[-*]\\s*`?(lint|typecheck|test|build)`?\\s*[:\\-]")) as $has_validation_checks
        | ($description_trimmed | test("(?im)^##\\s*metadata rationale\\b")) as $has_metadata_section
        | (($description_trimmed | test("(?im)\\bpriority\\b\\s*[:=]")) and ($description_trimmed | test("(?im)\\bestimate(dpoints)?\\b\\s*[:=]"))) as $has_metadata_values
        | ($has_description and $has_goal_or_context and $has_scope_signal and ($has_acceptance_heading or $has_acceptance_checklist) and ($has_validation_heading or $has_validation_checks) and ($has_metadata_section or $has_metadata_values) and ($has_human_required | not)) as $structural_readiness_ready
        | ($has_ralph and $has_prd_ready and ($has_human_required | not)) as $label_migration_readiness_ready
        | ($structural_readiness_ready or $label_migration_readiness_ready) as $readiness_ready
        | (
            if $structural_readiness_ready then "structural"
            elif $label_migration_readiness_ready then "label_migration"
            else "not_ready"
            end
          ) as $readiness_source
        | (
        ...
        | {
            ...
            readinessDiagnostics: {
              ...
              structural_ready: $structural_readiness_ready,
              label_migration_ready: $label_migration_readiness_ready,
            ...
            scheduleDecision: {
              ...
              tuple: {
                ...
                structural_readiness_ready: $structural_readiness_ready,
                label_migration_readiness_ready: $label_migration_readiness_ready,
...
validate_result_contract() {
  ...
  node "$CLI_RESULT_VALIDATOR" "$result_path" >/dev/null
}
...
write_loop_state() {
  ...
  jq -n \
    ...
    --arg calibration_path "$CALIBRATION_ABS" \
    --arg workflow_mode "$WORKFLOW_MODE" \
    --arg dispatch_mode "$DISPATCH_MODE" \
    --arg dispatch_id "$DISPATCH_ID" \
    --arg dispatch_run_id "$DISPATCH_RUN_ID" \
    --arg dispatch_log_path "${DISPATCH_LOG_ABS:-}" \
    ...
    --arg process_calibration_update "$PROCESS_CALIBRATION_UPDATE" \
    --arg calibration_update_cmd "$CALIBRATION_UPDATE_CMD" \
    ...
    --argjson calibration_summary "$CALIBRATION_SUMMARY_JSON" \
    ...
    '
    {
      ...
      options: {
        ...
        dispatch_run_id: (if $dispatch_run_id == "" then null else $dispatch_run_id end),
        dispatch_log_path: (if $dispatch_log_path == "" then null else $dispatch_log_path end),
        resume: ($resume == "true"),
        ...
        process_calibration_update: ($process_calibration_update == "true"),
        calibration_update_cmd: $calibration_update_cmd,
...
load_resume_state() {
  ...
  prior_workflow_mode="$(jq -r '.options.workflow_mode // "independent"' "$LOOP_STATE_ABS")"
  ...
  if [ -n "$prior_workflow_mode" ] && [ "$prior_workflow_mode" != "$WORKFLOW_MODE" ]; then
    fail "loop-state workflow_mode mismatch: state=$prior_workflow_mode current=$WORKFLOW_MODE"
  fi
  ...
  CALIBRATION_SUMMARY_JSON="$(jq -c '.calibration // {"updated":false,"path":null,"samples_total":0,"usable_samples":0,"tokens_per_point":null,"worker_exit_code":null}' "$LOOP_STATE_ABS")"
...
run_calibration_update() {
  ...
  local calibration_output_file=""
  local calibration_output=""
  local timestamp=""

  if [ "$PROCESS_CALIBRATION_UPDATE" != "true" ]; then
    CALIBRATION_SUMMARY_JSON="$(jq -n -c --arg path "$CALIBRATION_ABS" '
      {
        updated: false,
        path: $path,
        samples_total: 0,
  ...
  calibration_output_file="$(mktemp)"
  set +e
  "${CALIBRATION_UPDATE_CMD_ARR[@]}" \
    --retrospective "$RETROSPECTIVE_ABS" \
    --output "$CALIBRATION_ABS" > "$calibration_output_file" 2>&1
  calibration_exit=$?
  set -e

  calibration_output="$(cat "$calibration_output_file" 2>/dev/null || true)"
  rm -f "$calibration_output_file"

  if [ "$calibration_exit" -eq 0 ] && [ -n "$calibration_output" ] && jq -e '.' >/dev/null 2>&1 <<< "$calibration_output"; then
    CALIBRATION_SUMMARY_JSON="$(jq -c --argjson worker_exit_code "$calibration_exit" '
      . + {worker_exit_code: $worker_exit_code}
    ' <<< "$calibration_output")"
  else
    CALIBRATION_SUMMARY_JSON="$(jq -n -c \
      --arg path "$CALIBRATION_ABS" \
      --arg output "$calibration_output" \
      --argjson worker_exit_code "$calibration_exit" \
      '
      {
        updated: false,
        path: $path,
        ...
        tokens_per_point: null,
        worker_exit_code: $worker_exit_code,
        mode: "updater_failure",
        worker_output: $output
      }')"
  fi

  timestamp="$(now_iso)"
  echo "RUN_CALIBRATION timestamp=$timestamp updated=$(jq -r '.updated // false' <<< "$CALIBRATION_SUMMARY_JSON") samples_total=$(jq -r '.samples_total // 0' <<< "$CALIBRATION_SUMMARY_JSON") usable_samples=$(jq -r '.usable_samples // 0' <<< "$CALIBRATION_SUMMARY_JSON") tokens_per_point=$(jq -r '.tokens_per_point // "null"' <<< "$CALIBRATION_SUMMARY_JSON") worker_exit=$(jq -r '.worker_exit_code // "null"' <<< "$CALIBRATION_SUMMARY_JSON")" | tee -a "$PROGRESS_ABS"
}
...
if [ "$DISPATCH_MODE" = "orchestrated" ]; then
  echo "RUN_START timestamp=$(now_iso) prd=$PRD_ABS dispatch_mode=orchestrated dispatch_id=$DISPATCH_ID dispatch_run_id=$DISPATCH_RUN_ID workflow_mode=$WORKFLOW_MODE" | tee -a "$PROGRESS_ABS"

  issue_count="$(jq '[.issues[]] | length' "$PRD_ABS")"
  pending_issues_json="$(jq -c '[.issues[] | select((.passes // false) != true)]' "$PRD_ABS")"
  pending_issue_count="$(jq 'length' <<< "$pending_issues_json")"
  if [ "$pending_issue_count" -eq 0 ]; then
    fail "orchestrated mode: no pending issues in PRD"
  fi
  if [ "$pending_issue_count" -gt 1 ]; then
    fail "orchestrated mode: expected exactly 1 pending issue, found $pending_issue_count"
  fi

  issue_json="$(jq -c '.[0]' <<< "$pending_issues_json")"
  issue_id="$(jq -r '.issueId // ""' <<< "$issue_json")"
  issue_title="$(jq -r '.title // ""' <<< "$issue_json")"
  issue_description="$(jq -r '.description // ""' <<< "$issue_json")"
  issue_estimated_points_json="$(jq -c '.estimatedPoints // .estimate // null' <<< "$issue_json")"
  issue_priority_json="$(jq -c '.priority // null' <<< "$issue_json")"
  linear_issue_id="$(jq -r '.linearIssueId // ""' <<< "$issue_json")"
  safe_issue_id="${issue_id//[^A-Za-z0-9._-]/_}"

  [ -n "$issue_id" ] || fail "orchestrated mode: pending issue has no issueId"

  emit_dispatch_event "claim" "$(jq -n -c \
    --arg issue_id "$issue_id" \
    --argjson attempt 1 \
    --arg workflow_mode "$WORKFLOW_MODE" \
    '{issue_id: $issue_id, attempt: $attempt, workflow_mode: $workflow_mode}')"

  payload_path="$RESULTS_ABS/${safe_issue_id}-iter-1-input.json"
  result_path="$RESULTS_ABS/${safe_issue_id}-iter-1-result.json"
  jq -n \
    --arg contract_version "1.0" \
    --argjson iteration 1 \
    --arg issue_id "$issue_id" \
    ...
    --arg workflow_mode "$WORKFLOW_MODE" \
    --arg dispatch_mode "orchestrated" \
    --arg dispatch_id "$DISPATCH_ID" \
    --arg dispatch_run_id "$DISPATCH_RUN_ID" \
    --arg run_log_path "$RUN_LOG_ABS" \
    --arg progress_path "$PROGRESS_ABS" \
  ...
  emit_dispatch_event "heartbeat" "$(jq -n -c \
    --arg issue_id "$issue_id" \
    --arg phase "execution" \
    --arg summary "Starting agent execution" \
    '{issue_id: $issue_id, phase: $phase, summary: $summary}')"

  start_epoch_ms=$(( $(date +%s) * 1000 ))
  set +e
  RALPH_WORKFLOW_MODE="$WORKFLOW_MODE" \
  RALPH_DISPATCH_MODE="orchestrated" \
  RALPH_DISPATCH_ID="$DISPATCH_ID" \
  ...
  emit_dispatch_event "complete" "$(jq -n -c \
    --arg issue_id "$issue_id" \
    --argjson attempt 1 \
    --arg outcome "$outcome" \
    --argjson exit_code "$result_exit_code" \
    ...
    '{
      ...
      outcome: $outcome,
      exit_code: $exit_code,
      failure_category: (if $failure_category == "none" then null else $failure_category end),
      retryable: $retryable,
      result_path: $result_path
    }')"

  echo "RUN_ITERATION timestamp=$(now_iso) iteration=1 issue=$issue_id outcome=$outcome dispatch_mode=orchestrated dispatch_id=$DISPATCH_ID" | tee -a "$PROGRESS_ABS"
  echo "RUN_COMPLETE timestamp=$(now_iso) iterations=1 completed=$(if [ "$outcome" = "success" ]; then echo 1; else echo 0; fi) pending=$(if [ "$outcome" = "success" ]; then echo 0; else echo 1; fi) stop_reason=dispatch_single_issue dispatch_mode=orchestrated" | tee -a "$PROGRESS_ABS"
...
echo "RUN_START timestamp=$(now_iso) prd=$PRD_ABS max=$MAX_ITERATIONS resume=$RESUME workflow_mode=$WORKFLOW_MODE dispatch_mode=$DISPATCH_MODE review_feedback_sweep=$PROCESS_REVIEW_FEEDBACK_SWEEP loop_state=$LOOP_STATE_ABS run_log_enabled=$RUN_LOG_ENABLED" | tee -a "$PROGRESS_ABS"
...
while [ "$iterations" -lt "$MAX_ITERATIONS" ]; do
  ...
  failure_category="$(jq -r '.failure_category // "none"' "$result_path")"
  summary="$(jq -r '.summary' "$result_path")"
  tokens_used_result_json="$(jq -c '.metrics.tokens_used // null' "$result_path")"
  token_usage_result_json="$(jq -c '.metrics.token_usage // null' "$result_path")"
  tokens_used=0
  tokens_used_for_log="null"
  token_telemetry_available="false"
  token_usage_for_log="null"

  if [ "$token_usage_result_json" != "null" ] && jq -e '.source and .source != "unavailable"' >/dev/null 2>&1 <<< "$token_usage_result_json"; then
    tokens_used="$(jq -r '.total_tokens // 0' <<< "$token_usage_result_json")"
    tokens_used_for_log="$tokens_used"
    token_telemetry_available="true"
    token_usage_for_log="$token_usage_result_json"
  elif [[ "$tokens_used_result_json" =~ ^[0-9]+$ ]]; then
    tokens_used="$tokens_used_result_json"
    tokens_used_for_log="$tokens_used_result_json"
    token_telemetry_available="true"
    if [ "$token_usage_result_json" != "null" ]; then
      token_usage_for_log="$token_usage_result_json"
    fi
  fi

  if [ "$token_usage_for_log" = "null" ] && [ "$token_telemetry_available" = "true" ] && [ "$tokens_used" -gt 0 ]; then
    token_usage_for_log="$(jq -n -c --argjson total "$tokens_used" '
      {input_tokens: 0, output_tokens: 0, total_tokens: $total, source: "codex_api"}
    ')"
  fi
  ...
  STAGE_TELEMETRY_JSON="$(jq -c \
    ...
    --argjson token_usage "$token_usage_for_log" \
    --argjson failed_checks "$validation_failed_checks" \
    '
    .execution.attempts += 1
    | .execution.tokens_used += $tokens
    | if $token_usage != null and $token_usage.source != "unavailable" then
        .execution.input_tokens_used = ((.execution.input_tokens_used // 0) + ($token_usage.input_tokens // 0))
        | .execution.output_tokens_used = ((.execution.output_tokens_used // 0) + ($token_usage.output_tokens // 0))
        | .execution.telemetry_reported_count = ((.execution.telemetry_reported_count // 0) + 1)
      else
        .execution.telemetry_unavailable_count = ((.execution.telemetry_unavailable_count // 0) + 1)
      end
  ...
  if [ "$RUN_LOG_ENABLED" = "true" ]; then
    jq -n -c \
      ...
      --arg summary "$summary" \
      --argjson duration_ms "$duration_ms" \
      --argjson tokens_used "$tokens_used_for_log" \
      --argjson token_telemetry_available "$token_telemetry_available" \
      --argjson token_usage "$token_usage_for_log" \
      --argjson estimated_points "$issue_estimated_points_json" \
      ...
      '
      {
        ...
        tokenTelemetryAvailable: $token_telemetry_available,
  ...
  LAST_ITERATION_JSON="$(jq -n -c \
    ...
    '
    {
      ...
      token_telemetry_available: $token_telemetry_available,
      token_usage: $token_usage
    }')"
...
run_calibration_update
calibration_updated="$(jq -r '.updated // false' <<< "$CALIBRATION_SUMMARY_JSON")"
calibration_samples_total="$(jq -r '.samples_total // 0' <<< "$CALIBRATION_SUMMARY_JSON")"
calibration_usable_samples="$(jq -r '.usable_samples // 0' <<< "$CALIBRATION_SUMMARY_JSON")"
calibration_tokens_per_point="$(jq -r '.tokens_per_point // "null"' <<< "$CALIBRATION_SUMMARY_JSON")"
calibration_worker_exit="$(jq -r '.worker_exit_code // "null"' <<< "$CALIBRATION_SUMMARY_JSON")"
