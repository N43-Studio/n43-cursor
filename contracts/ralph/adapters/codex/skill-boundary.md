...
| Run single-entry setup flow (`create-project` -> `audit-project`) | `ralph-build` |
| Create exactly one implementation-ready Issue within the Ralph contract workflow | `ralph-create-issue` |
...
- Linear PM -> Ralph:
  - Once project/issue structure is ready, hand off to `ralph-create-issue` or the `ralph-create-project`/`ralph-populate-project` pipeline.
- Ralph -> Linear PM:
